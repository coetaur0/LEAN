"""
Definition of custom layers for the LEAN model.
"""
# Aurelien Coet, 2018.
...
class LEAttention(nn.Module):
    ...
    The dot product of the encoded vectors in the premises and hypotheses is
    first computed.
    Then, the resulting matrix is combined with the Word2Hyp and LEAR entailment
    matrices the model receives as input in a weighted sum.
    Finally, the softmax of the result is used in a weighted sum of the vectors
    of the premises for each element of the hypotheses, and conversely for the
    ...
    def __init__(self):
        ...
        self.lambda1 = nn.Parameter(torch.tensor(np.random.normal(),
                                    dtype=torch.float))
        self.lambda2 = nn.Parameter(torch.tensor(np.random.normal(),
                                    dtype=torch.float))
    ...
    def forward(self,
        ...
        attention_matrix = similarity_matrix\
                         + self.lambda1 * w2h_scores\
                         + self.lambda2 * lear_scores

        # Softmax attention weights.
        prem_hyp_attn = masked_softmax(attention_matrix, hypothesis_mask)
        hyp_prem_attn = masked_softmax(attention_matrix.transpose(1, 2)
                                                       .contiguous(),
                                       premise_mask)
