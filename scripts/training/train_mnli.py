"""
Train the LEAN model on the preprocessed MultiNLI dataset.
"""
# Aurelien Coet, 2019.
...
from lean.data import NLIDataset
from lean.model import LEAN1
...
def main(train_file,
         ...
         checkpoint=None):
    """
    Train the LEAN model on the SNLI dataset.
    ...
    Args:
        ...
        w2h_file: The path to a file containing preprocessed Word2Hyp
            word embeddings that must be used in the model.
        lear_file: The path to a file containing preprocessed LEAR
            word embeddings that must be used in the model.
    ...
    with open(w2h_file, "rb") as pkl:
        w2h_embeddings = torch.tensor(pickle.load(pkl), dtype=torch.float)\
                         .to(device)
    
    with open(lear_file, "rb") as pkl:
        lear_embeddings = torch.tensor(pickle.load(pkl), dtype=torch.float)\
                          .to(device)

    model = LEAN1(embeddings.shape[0],
                  embeddings.shape[1],
                  hidden_size,
                  embeddings=embeddings,
                  w2h_embeddings=w2h_embeddings,
                  lear_embeddings=lear_embeddings,
                  dropout=dropout,
    ...
    print("\n",
          20 * "=",
          "Training LEAN model on device: {}".format(device),
          20 * "=")
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        torch.save({"epoch": epoch,
                    ...
                    "match_valid_losses": matched_valid_losses,
                    "mismatch_valid_losses": mismatched_valid_losses},
                   os.path.join(target_dir, "lean_{}.pth.tar".format(epoch)))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train the LEAN model on MultiNLI")
    ...
    main(os.path.normpath(os.path.join(script_dir, config["train_data"])),
         os.path.normpath(os.path.join(script_dir, config["valid_matched"])),
         os.path.normpath(os.path.join(script_dir, config["valid_mismatched"])),
         os.path.normpath(os.path.join(script_dir, config["embeddings"])),
         os.path.normpath(os.path.join(script_dir, config["w2h_embeddings"])),
         os.path.normpath(os.path.join(script_dir, config["lear_embeddings"])),
