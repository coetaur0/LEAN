"""
Test the LEAN model on the preprocessed MultiNLI dataset.
"""
# Aurelien Coet, 2019.
...
def main(test_matched, test_mismatched, pretrained_file, labeldict, output_dir, batch_size=32):
    """
    Test the LEAN model with pretrained weights on the MultiNLI dataset.
    ...
    with open(os.path.normpath(test_matched), 'rb') as pkl:
        matched_test_data = NLIDataset(pickle.load(pkl))
    with open(os.path.normpath(test_mismatched), 'rb') as pkl:
    ...
    print(20 * "=",
          " Prediction on MNLI with LEAN model on device: {} ".format(device),
          20 * "=")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test the LEAN model on\
 the MNLI matched and mismatched test sets')
    ...
    main(os.path.normpath(os.path.join(script_dir, config['test_matched'])),
         os.path.normpath(os.path.join(script_dir, config['test_mismatched'])),
         args.checkpoint,
         config['labeldict'],
         os.path.normpath(os.path.join(script_dir, config['output_dir'])),
         config['batch_size'])
